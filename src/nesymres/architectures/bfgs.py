...
def evaluate_formula_samples(formula: str, X: torch.Tensor) -> torch.Tensor:
    ...
    """
    formula = make_human_readable(str(formula))
    # print("utils formula", formula)
...
def make_human_readable(infix_expr):
    ...
    # Replace operators in the infix expression with human-readable symbols
...
def bfgs(pred_str, X, y, cfg, verbose=False):
    ...
    if cfg.bfgs.add_coefficients_if_not_existing and 'constant' not in raw:
        return Generator.prefix_to_infix(raw, 
    ...
    # print(type(expr))
    try:
        expr = make_human_readable(expr)
    except Exception as e:
        print("BFGS MAKE HUMAN READABLE", expr, type(expr), e)
    ...
    for i in range(cfg.bfgs.n_restarts):
        ...
        # y_found = sp.lambdify(",".join(cfg.total_variables), final)(**values) # Arco: OG
        tensor_values = [val.clone().detach() for val in values.values()]
        values = torch.stack(tensor_values, dim=1)
        y_found = evaluate_formula_samples(final, values)
