...
class Generator(object):
    ...
    def _generate_expr(
        ...
        def extract_variables(expr):
            # Use a set comprehension to collect unique variables starting with 'x_'
            return list({item for item in expr if item.startswith("x_")})
        
        def mutate_formula(formula, chance=0.5, mutation_range=(-1, 5)):
            for i, item in enumerate(reversed(formula)):
                idx = len(formula) - 1 - i
                if item.startswith('x_'):
                    # print(item)
                    if rng.uniform(0, 1) < chance:  # Apply mutation with given chance
                        formula[idx] = str(rng.randint(*mutation_range))
                    return formula
            return formula
        """
        Create a tree with exactly `nb_total_ops` operators.
        """
        # print(nb_total_ops) # TO BE DELETED
        stack = [None]
        ...
        if generate_simple_example:
            for _ in range(mutation_times):
                stack = mutate_formula(stack, mutation_prop, mutation_range=rangee)
            variables = extract_variables(stack)
    ...
    def generate_equation(self, rng):
        ...
        return f_prefix, infix, variables
